            ...
            <div id="messages">
                <?php foreach ($messages as $message): ?>
                    <div class="message <?php echo $message['sender_id'] === $user_id ? 'sent' : 'received'; ?>">
                        ...
                        <small class="timestamp"><?php echo htmlspecialchars(date('H:i', strtotime($message['timestamp']))); ?></small>
                        <small class="timestamp"><?php echo htmlspecialchars(date('y-m-d', strtotime($message['timestamp']))); ?></small>
                     </div>
                     <?php endforeach; ?>
                    </div>
        ...
        function sendMessage() {
            ...
            if (message.length > 0) {
                fetch('send_message.php', {
                    ...
                    messagesContainer.innerHTML += `<div class="message sent">${message}</div>`;
            ...
            fetch(`get_chat.php?user_id=${chatUserId}`)
                ...
                .then(messages => {
                    ...
                    messages.forEach(message => {
                        ...
                        chatHtml += `<div class="message ${messageClass}">${message.message}</div>`;
                    });
